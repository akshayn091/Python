...
class SimpleLoadBalancer(object):
    ...
    def update_lb_mapping(self, client_ip):
        ...
        if client_ip not in self.flow_table.keys():  # 1
            self.flow_table[client_ip] = random.choice(self.server_arp_table.keys()), time.time()
            log.info("no flow for client_ip")
        if client_ip in self.flow_table.keys():  # 2
			if time.time() > self.flow_table[client_ip][1]:
				self.flow_table[client_ip] = random.choice(self.server_arp_table.keys()), time.time()
				# self.flow_table.update({client_ip: random.choice(self.server_ips)}, datetime.datetime.now())
				log.info("flow for client_ip has expired")
			else:  # 3
				self.flow_table[client_ip] = client_ip, time.time()
				# self.flow_table[client_ip] = datetime.datetime.now()
				log.info("flow for client_ip has not expired and updated with latest time")
    ...
    def send_proxied_arp_reply_to_server(self, arp_pkt):
        ...
        r.protosrc = arp_pkt.protodst  # the source IP in arp reply
        e = ethernet(type=ethernet.ARP_TYPE, src=self.lb_mac,
                     dst=ETHER_BROADCAST)
        e.set_payload(r)
